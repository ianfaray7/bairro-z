...
public class enemyVoador : MonoBehaviour
{
    ...
    void Start()
    {
        if (enemyVoadorManager.main == null || enemyVoadorManager.main.checkpoints == null || enemyVoadorManager.main.checkpoints.Length == 0)
        {
            Debug.LogError("enemyVoadorManager.main.checkpoints não configurado.", this);
            enabled = false;
            return;
        }

        index = Mathf.Clamp(index, 0, enemyVoadorManager.main.checkpoints.Length - 1);
        checkpoint = enemyVoadorManager.main.checkpoints[index];
    ...
    void Update()
    {
        ...
        {
            index++;
            if (index >= enemyVoadorManager.main.checkpoints.Length)
            {
    ...
    #if UNITY_EDITOR
        Debug.Log($"[enemyVoador] {gameObject.name} update walk to {targetState} (dir={dir.x:F2},{dir.y:F2}) flipX={spriteRenderer?.flipX}");
    #endif
    }
