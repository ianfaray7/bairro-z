...
public class UIManager : MonoBehaviour
{
    ...
    private bool victorySubscribed = false;
    void Start()
    {
        ...
        if (ResourceManager.Instance != null)
        {
            ...
            ResourceManager.Instance.OnVictory.AddListener(ShowVictoryPanel);
            victorySubscribed = true;
            Debug.Log("UIManager: subscribed to ResourceManager.OnVictory");
        }
        else
        {
    ...
    void FixedUpdate()
    {
        ...
        if (!victorySubscribed && ResourceManager.Instance != null)
        {
            ResourceManager.Instance.OnVictory.AddListener(ShowVictoryPanel);
            victorySubscribed = true;
            Debug.Log("UIManager: deferred subscription to ResourceManager.OnVictory");
        }
    ...
    {
        Debug.Log("UIManager: ShowVictoryPanel called");
        ...
        if (victoryPanel != null)
        {
            ...
            PauseMenu.HideAllPauseUI();
            // wire first found button to return to menu
            var btn = victoryPanel.GetComponentInChildren<Button>(true);
            if (btn != null)
            {
                btn.onClick.RemoveAllListeners();
                btn.onClick.AddListener(() => { PauseManager.Resume(); UnityEngine.SceneManagement.SceneManager.LoadScene(0); });
    ...
    private void CreateRuntimeVictoryPanel()
    {
        ...
        title.fontSize = 42;
        var fnt = UnityEngine.Resources.GetBuiltinResource<UnityEngine.Font>("LegacyRuntime.ttf"); 
        if (fnt == null) fnt = UnityEngine.Resources.GetBuiltinResource<UnityEngine.Font>("Arial.ttf"); 
        if (fnt != null) title.font = fnt;

        var btnGo = new GameObject("ReturnToMenu"); btnGo.transform.SetParent(container.transform,false);
        var bimg = btnGo.AddComponent<UnityEngine.UI.Image>(); bimg.color = new Color(0.8f,0.6f,0f,1f);
        var btn = btnGo.AddComponent<UnityEngine.UI.Button>();
        var t = new GameObject("Text"); t.transform.SetParent(btnGo.transform,false);
        var tt = t.AddComponent<UnityEngine.UI.Text>(); 
        tt.text = "Voltar ao Menu"; 
        tt.alignment = TextAnchor.MiddleCenter; 
        tt.color = Color.white; 
        tt.fontSize = 20; 
        if (fnt != null) tt.font = fnt;
        var le = btnGo.AddComponent<UnityEngine.UI.LayoutElement>(); le.preferredWidth = 300; le.preferredHeight = 50;
        btn.onClick.AddListener(() => { PauseManager.Resume(); UnityEngine.SceneManagement.SceneManager.LoadScene(0); });
